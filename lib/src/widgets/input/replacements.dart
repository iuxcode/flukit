...
/// in a [FluTextEditingInlineSpanReplacement].
///
...
///
/// This used in [FluReplacementTextEditingController] to generate [InlineSpan]s when
...
/// See also:
///
/// * [FluReplacementTextEditingController], which uses this class to create
/// rich text fields.
/// {@end-tool}
class FluTextEditingInlineSpanReplacement {
  /// Constructs a replacement that replaces matches of the [TextRange] with the
  /// output of the [generator].
  FluTextEditingInlineSpanReplacement(this.range, this.generator, this.expand);
  ...
  List<FluTextEditingInlineSpanReplacement>? onReplacement(
  ...
  FluTextEditingInlineSpanReplacement copy({TextRange? range, bool? expand}) {
    return FluTextEditingInlineSpanReplacement(
        range ?? this.range, generator, expand ?? this.expand);
  }

  @override
  String toString() {
    return 'FluTextEditingInlineSpanReplacement { range: $range, generator: $generator }';
  }
}

/// A [TextEditingController] that contains a list of [FluTextEditingInlineSpanReplacement]s that
/// insert custom [InlineSpan]s in place of matched [TextRange]s.
///
/// This controller must be passed [FluTextEditingInlineSpanReplacement], each of which contains
...
///
/// See [FluTextEditingInlineSpanReplacement] for example replacements to provide this class with.
class FluReplacementTextEditingController extends TextEditingController {
  ...
  FluReplacementTextEditingController.fromValue(super.value,
      {List<FluTextEditingInlineSpanReplacement>? replacements,
      this.composingRegionReplaceable = true})
      : super.fromValue();

  /// The [FluTextEditingInlineSpanReplacement]s that are evaluated on the editing value.
  ///
  ...
  void applyReplacement(FluTextEditingInlineSpanReplacement replacement) {
  ...
  void syncReplacementRanges(TextEditingDelta delta) {
    ...
    List<FluTextEditingInlineSpanReplacement> toRemove = [];
    List<FluTextEditingInlineSpanReplacement> toAdd = [];

    for (int i = 0; i < replacements!.length; i++) {
      late final FluTextEditingInlineSpanReplacement? mutatedReplacement;
      ...
      if (delta is TextEditingDeltaInsertion) {
        ...
        List<FluTextEditingInlineSpanReplacement>? newReplacements;
  ...
  TextSpan buildTextSpan({
    ...
    return TextSpan(
      style: style,
      children:
          spans.map((span) => Flu.replaceEmojis(span as TextSpan)).toList(),
    );
  }
  ...
  void disableExpand(TextStyle style) {
    ...
    for (final FluTextEditingInlineSpanReplacement replacementWithExpandDisabled
        in toAdd) {
  ...
  void removeReplacementsAtRange(TextRange removalRange, TextStyle? attribute) {
    ...
    for (int i = 0; i < replacements!.length; i++) {
      FluTextEditingInlineSpanReplacement replacement = replacements![i];
    ...
    for (FluTextEditingInlineSpanReplacement replacementToAdd in toAdd) {
      replacements!.add(replacementToAdd);
    }

    for (FluTextEditingInlineSpanReplacement replacementToRemove in toRemove) {
      replacements!.remove(replacementToRemove);
    }
  }
}
