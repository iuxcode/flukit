...
extension FluSecureStorageService on FluInterface {
...
class FluStorageService {
  ...
  Future<bool> containsKey(String key) async {
    var containsKey = await _secureStorage.containsKey(
        key: key, aOptions: getAndroidOptions());
    return containsKey;
  }
  ...
  Future<String?> read(String key) async {
    var readData =
        await _secureStorage.read(key: key, aOptions: getAndroidOptions());
    return readData;
  }
  ...
  Future<List<FluSecureStorageItem>> readAll() async {
    ...
    List<FluSecureStorageItem> list = allData.entries
        .map((e) => FluSecureStorageItem(e.key, e.value))
        .toList();
    return list;
  }
...
enum FluAuthorizationStates { waitAuth, waitCode, waitTerms, ready }
